...
@multitier object NotSimpleTimeService {
  @peer type Peer
...
object Server extends App {
  multitier start new Instance[NotSimpleTimeService.Server](
    listen[NotSimpleTimeService.Client] { TCP(43053) })
}

object Client extends App {
  multitier start new Instance[NotSimpleTimeService.Client](
    connect[NotSimpleTimeService.Server] { TCP("localhost", 43053) })
}