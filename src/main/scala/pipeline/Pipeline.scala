...
case class Tweet(tags: Set[String], author: Author) {
  def hasHashtag(tag: String): Boolean = tags contains tag
}
...
//  on[Folder] {
//    println("subscribing to folded signal")
//    folded observe { result =>
//      println(s"result: ${
//        if (result.isEmpty) "(none)"
//        else result map { case (author, count) => s"${author.name} ($count)" } mkString ","
//      }")
//    }
//  }
//
